# Install and integrate Keysso MCP

## Install a release package

Install [uv](https://docs.astral.sh/uv/getting-started/installation/) and download the
wheel `zai_keysso_mcp-0.3.0-py3-none-any.whl` plus `SHA256SUMS.txt` from [v0.3.0](https://github.com/zai-one/keysso-mcp/releases/tag/v0.3.0).
...
```sh
uv tool install "zai-keysso-mcp @ https://github.com/zai-one/keysso-mcp/releases/download/v0.3.0/zai_keysso_mcp-0.3.0-py3-none-any.whl"
keysso-mcp-setup --directory /absolute/path/to/my-keysso-settings
keysso-mcp --config /absolute/path/to/my-keysso-settings/mcp.local.json --check-config
```
...
Windows accepts paths such as `C:/Users/you/mcp/keysso`; quote paths containing
...
```sh
keysso-mcp-setup --directory /absolute/path/to/my-keysso-settings --client-only
keysso-mcp-setup --directory /absolute/path/to/my-keysso-settings --client-only --format toml
```
...
`C:/Users/you/mcp/keysso-mcp`). Give the MCP client this stdio configuration:
...
```sh
uv run --frozen keysso-mcp --config mcp.local.json --transport http --check-config
uv run --frozen keysso-mcp --config mcp.local.json --transport http --host 127.0.0.1
```

Configure `KEYSSO_MCP_PUBLIC_KEY_FILE` with an RSA verification key and issue
...
```sh
uv build
python -m pip install "dist/zai_keysso_mcp-0.3.0-py3-none-any.whl"
keysso-mcp --config /ABSOLUTE/PATH/mcp.local.json
```
